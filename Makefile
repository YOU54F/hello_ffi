...
alpine_haskell:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add ghc make musl-dev && cd /app && make haskell_hello_ffi'
...
alpine_python:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make python3 python3-dev py-pip gcc musl-dev libffi-dev && cd /app && python3 -m venv /home/venv && . /home/venv/bin/activate && make python_install_deps && make python'
...
alpine_ruby:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make ruby ruby-dev ruby-bundler build-base libffi-dev && cd /app && make ruby_install_deps && make ruby'
...
alpine_raku:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make rakudo gcc && cd /app && make raku'
...
alpine_racket:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make racket gcc && cd /app && make racket'
...
alpine_julia: # needs an x86_64 binary
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} --platform=linux/amd64 -v ${PWD}:/app --rm julia:alpine sh -c 'apk add make gcc musl-dev && cd /app && make julia'
...
alpine_csharp:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make && apk add mono --repository=http://dl-cdn.alpinelinux.org/alpine/edge/testing/ && cd /app && make csharp'
...
alpine_bun:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm oven/bun:alpine sh -c 'apk add make && cd /app && make bun'

# alpine_bun:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make bash curl patchelf gcc libffi libffi-dev musl-dev && curl -LO https://raw.githubusercontent.com/YOU54F/bun-musl/main/bun-musl && chmod +x bun-musl && ./bun-musl install && export PATH=$$PATH:$$HOME/.bun/bin && cd /app && make bun'
...
alpine_dart:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make ca-certificates && apk add dart --repository=http://dl-cdn.alpinelinux.org/alpine/edge/testing/ && cd /app && make dart_hello_ffi'
...
alpine_c:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make gcc musl-dev && cd /app && make c'
...
# alpine_swift:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make swift && cd /app && make swift'
...
alpine_lua:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make lua luajit protoc && cd /app && make lua'
...
# TODO
# alpine_scala_native:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make && cd /app && make scala_native'
...
# TODO
# alpine_scala:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make && cd /app && make scala'
...
alpine_nim:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make nim gcc musl-dev && cd /app && make nim'
...
alpine_visual_basic:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make && apk add mono --repository=http://dl-cdn.alpinelinux.org/alpine/edge/testing/  && cd /app && VBC_COMPILER=vbc make visual_basic'
...
# alpine_kotlin:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make openjdk11 && apk add gradle --repository=http://dl-cdn.alpinelinux.org/alpine/edge/testing/ && cd /app && make kotlin'
kotlin_hello_ffi:
...
alpine_ocaml:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make ocaml gcc musl-dev && apk add ocaml-utop ocaml-utop-dev --repository=http://dl-cdn.alpinelinux.org/alpine/edge/testing/ && cd /app && make ocaml'
ocaml_hello_ffi:
...
# alpine_tcl:
# 	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make tcl tcl-dev gcc musl-dev && cd /app && make tcl'
# tcl_hello:
# 	tclsh tcl/hello.tcl
...
alpine_java_panama:
	docker run --platform=${DOCKER_DEFAULT_PLATFORM} -v ${PWD}:/app --rm alpine sh -c 'apk add make openjdk19 libgcc && cd /app && make java_panama'
java_panama_ffi_gen:
